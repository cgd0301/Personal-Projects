...
set every digit of max to 256
set min equals negative max
l = min
r = max
R = COPY(K)
while CMP(l, r, N) == SMALLER:
  mid = DIV(ADD(l, r, N), 2, N)
  COUNT = ZERO(N)
  mid_k = 1
  while CMP(mid_k, A, N) == SMALLER and CMP(COUNT, K, N) == SMALLER:
    mid_k = MULTIPLY(mid_k, mid, N)
    COUNT += 1
  if CMP(mid_k, A, N) == GREATER:
    r = mid
  else:
    l = ADD(mid, 1, N)
return SUBTRACT(l, 1, N)
