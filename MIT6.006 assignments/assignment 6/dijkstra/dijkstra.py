...
class PathFinder(object):
    ...
    def dijkstra(self, weight, nodes, source, destination):
        ...
        for node in nodes:
            if node != source:
                pair = NodeDistancePair(node, inf)
                Q.insert(pair)
                D[node] = pair
            else:
                pair = NodeDistancePair(node, 0)
                Q.insert(pair)
                D[node] = pair
        S = set()
        ancestor = {source: None}
        while destination not in S and len(Q) > 0:
            nodePair = Q.extract_min()
            node = nodePair.node
            distance = nodePair.distance
            for node_adj in node.adj:
                if node_adj not in S:
                    if D[node_adj].distance > distance + weight(node, node_adj):
                        update_distance = distance + weight(node, node_adj)
                        D[node_adj].distance = update_distance
                        Q.decrease_key(D[node_adj])
                        ancestor[node_adj] = node
            S.add(node)

        if D[destination].distance == inf:
            return ([], inf)
        else:
            path = [destination]
            tmp_ptr = ancestor[destination]
            while tmp_ptr is not None:
                path.insert(0, tmp_ptr)
                tmp_ptr = ancestor[tmp_ptr]
            return (path, len(S))
