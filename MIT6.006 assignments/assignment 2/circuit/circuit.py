...
class Simulation:
    ...
    def step(self):
        ...
        """ 
        step_time = self.avl_tree.min().time
        
        # Need to apply all the transitions at the same time before propagating.
        transitions = []
        while self.avl_tree.len > 0 and self.avl_tree.min().time == step_time:
          transition = self.avl_tree.pop()
        ...
        for transition in transitions:
          for gate in transition.gate.out_gates:
            ...
            self.avl_tree.append(Transition(gate, output, time))
    ...
    def run(self):
        ...
        for in_transition in sorted(self.in_transitions):
            self.avl_tree.append(Transition(in_transition[3], in_transition[2],
                                         in_transition[0]))
        while self.avl_tree.len > 0:
            self.step()
        self.probes.sort()
...
if __name__ == '__main__':
    import sys
    sim = Simulation.from_file(sys.stdin) #input file and save into variable sim
