...
Let's say the area of the triangle at depth n is $S(n)$, 
and the length of which is $L(n)$. The area of $S(n)$ equals
to $1/9 \times S(n - 1)$. The number of the increased triangle at 
specific level is equal to the number of edges under the 2D condition.
Therefore, the recursion formula of total size of area from 0 to 
level n is $TS(n) = E(n - 1) \times S(n) + TS(n - 1)$ and set $T(0)$
equal to $c$. According to calculation, $E(n) = 3 \times 4 ^ n$.
Therefore, the recursion formula is 
$TS(n) = 3 \times 4 ^ (n - 1) \times (c \times (1/9) ^ (n - 1)) + TS(n - 1)$
The formula is upper bounded by $32/5 \times c$, which is a constant.
Therefore, the asymptotic cost is theta(n).
